...
class Net(nn.Module):
    ...
    def __init__(self, input_size, hidden_size1, hidden_size2, output_size):
        ...
        self.hidden_layer_1 = nn.Linear( input_size, hidden_size1, bias=True)
        self.hidden_layer_2 = nn.Linear( hidden_size1, hidden_size2, bias=True)
        self.output_layer = nn.Linear( hidden_size2, output_size , bias=True)
...
class sepNet(nn.Module):
    ...
    def __init__(self, input_size, hidden_size1, hidden_size2, output_size):
        ...
        self.net1 = Net(int(input_size/2), hidden_size1, hidden_size2, output_size)  
        self.net2 = Net(int(input_size/2), hidden_size1, hidden_size2, output_size) 
...
def get_loss(model,device,initial_conditions,bs,x0,H0,dim,wholemat,evalmat,trainset=False,verbose=False):
    ...
    for count in range(0,len(mat),bs):
      ...
      x=x.to(device)
      loss,f1,f2,f3,f4=lossfuc(model,curmat,x,device,x0,H0,dim)
...
def train(net,bs,num_epoch,initial_conditions,device,wholemat,evalmat,x0,H0,dim,LR,patience,c1,c2,c3,c4):
    ...
    for epoch in range(num_epoch):
        ...
        for count in range(0,len(wholemat),bs):
            ...
            loss,f1,f2,f3,f4=lossfuc(net,mat,x,device,x0,H0,dim,c1,c2,c3,c4)  
            loss.backward()
...
def lossfuc(model,mat,x,device,x0,H0,dim,c1=1,c2=1,c3=1,c4=1,verbose=False):
    x1,x2=torch.unsqueeze(x[:,0],1),torch.unsqueeze(x[:,1],1)
    y1,y2=model.net1(x1),model.net2(x2)
    f3=(model(torch.tensor([[x0,x0]]).to(device))-torch.tensor([[H0]]).to(device))**2
    dHdq=torch.autograd.grad(y1, x1, grad_outputs=y1.data.new(y1.shape).fill_(1),create_graph=True)[0]
    dHdp=torch.autograd.grad(y2, x2, grad_outputs=y2.data.new(y2.shape).fill_(1),create_graph=True)[0]
    # dHdq=dH[:,0]
    # dHdp=dH[:,1]
    qprime=mat[:,2]
    pprime=mat[:,3]
