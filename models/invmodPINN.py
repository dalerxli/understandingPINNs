...
def lossfuc(model,mat,x,y,device,x0,H0,dim,c1=1,c2=1,c3=1,c4=1,verbose=False):
    ...
    dH=torch.autograd.grad(y, x, grad_outputs=y.data.new(y.shape).fill_(1),create_graph=True, allow_unused=True)[0] # x is 1+q, y is p.
    dHdq=dH[:,0]
    dHdp=dH[:,1]
    qprime=(mat[:,2]-mat[:,0])
    pprime=(mat[:,3]-mat[:,1])
    ...
    # f4=torch.mean((dHdq*qprime+dHdp*pprime)**2,dim=0) # the inverse modified hamiltonian is itself symplectic
    f4 = torch.zeros(1)
