...
def classicIntNN(z,h,net,device):
	## classical symplectic Euler scheme
	dim = int(len(z)/2)
	q=z[:dim]
	p=z[dim:]		
	fstage = lambda stg: h * get_grad(net, np.concatenate([q+stg,p]),device)[0]
...
def classicTrajectoryNN(z,h,net,N=1,device):
	...
	trj[:,0] = z.copy()
	for j in range(0,N):
		trj[:,j+1] = classicIntNN(trj[:,j].reshape(-1,1).copy(),h,net,device)
	return trj[:, :-1], trj[:, 1:]
